...
/** 解析服务器上的原始签批数据
 * @note 可自己拿到原始数据设置解析方法
 * @note 不支持扩展版
 * @param fieldValue : 原始签批数据
 * @return : 解析后的签批信息
 */
- (NSArray *)fieldValuesWithValue:(NSString *)fieldValue;
 ...
 * @return : 服务器所需要的签批信息
 */
- (NSString *)fieldValueFromValues:(NSArray *)fieldValues;
 ...
 * @return : 签批区域数据
 */
- (NSString *)fieldValueWithSignatureImageData:(NSData *)signatureImageData signatureRect:(CGRect)signatureRect userName:(NSString *)userName oldFieldValue:(NSString *)oldFieldValue;

#pragma mark - webService
/** 获取文件列表
 * @note : success的回调参数由message改为docLists
 * @param webService : NSString，服务器地址
 * @param success : 成功回调（docLists：文档列表。@[@{(字典1)@"dateTime" : @"日期时间", @"recordID" : @"ID", @"title" : @"标题", @"userName" : @"用户名"}, @{字典2}, ...]）
 * @param failure : 失败回调
 */
...
/** 保存文档
 * @param webService : NSString，服务器
 * @param recordID : NSString，文档ID
 * @param userName : NSString，用户名
 * @param dateTime : NSString，日期时间
 * @param docNo : NSString，文档编号
 * @param security : NSString，保密级别
 * @param draft : NSString，草稿
 * @param check : NSString，处理进度
 * @param title : NSString，文档标题
 * @param copyTo : NSString，抄送
 * @param subject : NSString，关键字
 * @param copies : NSUInteger，份数
 * @param success : 成功回调
 * @param failure : 失败回调
 */
...
/** 加载多个签名区域内容
 * @param webService : NSString，服务器
 * @param recordID : NSString，文档记录号
 * @param signatureInfo : @{fieldName1（NSString：签名区域名称） : userName1（NSString，用户名）,
 ...
 * @param recordID : 文档ID
 * @param userName : 用户名
 * @param fieldInfo : @{fieldName1（NSString：签名区域名称） : fieldValue1（NString，base64类型，签名区域值）,
...
/** 获取印章内容
 * @param webService : NSString，服务器
 * @param imageName : NSString，印章名称
 * @param userName : NSString，用户名
 * @param password : NSString，印章密码
 * @param success : 成功回调
 * @param failure : 失败回调
 */
 ...
 * @param sigantureType : 签名类型（0，手写签批；1，文字批注）
 ...
 * @param userName : 用户名
 * @param fieldName : 签批区域名称
 * @param fieldInfos : 签批数据集合@[@{@"data" : NSData<图片数据>, @"type" : NSString<@"0" 或 @"1"。0，手写签批；1，文字批注>, @"scale" : NSString<图片缩放比例>}, ...]
